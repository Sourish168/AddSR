...
def load_addsr_pipeline(args, accelerator, enable_xformers_memory_efficient_attention):
    ...
    unet = UNet2DConditionModel.from_pretrained(args.addsr_model_path, subfolder="unet")
    controlnet = ControlNetModel.from_pretrained(args.addsr_model_path, subfolder="controlnet", low_cpu_mem_usage=False)
...
def main(args, enable_xformers_memory_efficient_attention=True,):
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("AddSR")

    pipeline = load_addsr_pipeline(args, accelerator, enable_xformers_memory_efficient_attention)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--addsr_model_path", type=str, default='')
    ...
    parser.add_argument("--PSR_weight", type=float, default=0.5)
    parser.add_argument("--added_prompt", type=str, default="clean, high-resolution, 8k, extremely detailed, best quality, sharp")
