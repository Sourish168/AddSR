...
class StableDiffusionControlNetPipeline(DiffusionPipeline, TextualInversionLoaderMixin):
    ...
    def __call__(
        ...
        timesteps = torch.tensor([999, 749, 499, 249], device='cuda')
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                if t in [749, 499, 249]:
                    #print('yes')
                    weight = PSR_weight
                    z0_stu_SecStep_ = 1 / self.vae.config.scaling_factor * pred_z0
                    z0_stu_SecStep_ = z0_stu_SecStep_.to(next(self.vae.parameters()).dtype)
                    x0_stu_SecStep = self.vae.decode(z0_stu_SecStep_, return_dict=False)[0].clamp(-1, 1).detach()
                    image = (x0_stu_SecStep / 2 + 0.5) * weight + (1-weight) * LR 
    
                    # extract soft semantic label
                    ram_values = F.interpolate(image, size=(384, 384), mode='bicubic')
                    for i in range(ram_values.shape[0]):
                        ram_values[i] = ram_normalize(ram_values[i])
                    with torch.no_grad():
                        ram_image = ram_values.cuda().to(torch.float16)
                ...
                # print(str(t))
                if t == 249:
                    latents = self.scheduler.get_z0(latent_model_input, noise_pred, t)
                    break
                else:
                    latents = self.scheduler.get_z0(latent_model_input, noise_pred, t)
                    pred_z0 = latents
                    ...
                    if t == 999:
                        tt = torch.tensor([749], device='cpu')
                        latents = self.scheduler.add_noise(latents, noise, tt.long())
                    elif t == 749:
                        tt = torch.tensor([499], device='cuda')
                        latents = self.scheduler.add_noise(latents, noise, tt.long())
                    elif t == 499:
                        tt = torch.tensor([249], device='cuda')
                        latents = self.scheduler.add_noise(latents, noise, tt.long())
